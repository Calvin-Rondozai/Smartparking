...
const MyBookingScreen = ({ navigation }) => {
  ...
  // Simple timer effect - count up from timer_started, freeze when booking completed
  useEffect(() => {
      ...
      for (const booking of bookings) {
        if (booking && booking.id && booking.status === "active") {
          ...
          if (booking.timer_started && booking.completed_at) {
            ...
            const minutes = Math.floor((finalDuration % (1000 * 60 * 60)) / (1000 * 60));
    ...
    bookings.forEach((booking) => {
      ...
      if (isCurrentlyAvailable && !autoActions[`departure_${booking.id}`]) {
        ...
        if (booking.timer_started) {
          ...
          if (hours > 0) {
            parkingDuration = `${hours}h ${minutes}m ${seconds}s`;
          } else if (minutes > 0) {
            parkingDuration = `${minutes}m ${seconds}s`;
          } else {
            parkingDuration = `${seconds}s`;
          }
        }

        // Show departure notification with parking duration - but don't auto-complete
        Alert.alert(
  ...
  // Handle manual completion when user leaves
  const handleCompleteBooking = async (bookingId) => {
    Alert.alert(
      "Complete Booking",
      "Are you leaving the parking spot? This will complete your booking and calculate the final cost.",
      [
        { text: "No", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              console.log(
                `[MyBookingScreen] Manually completing booking ${bookingId}`
              );
              const response = await bookingAPI.completeActiveBooking(
                bookingId
              );
              console.log(`[MyBookingScreen] Complete response:`, response);
              Alert.alert(
                "Booking Completed",
                `Your parking session has ended. Final cost: $${
                  response.total_cost?.toFixed(2) || "0.00"
                }`,
                [
                  {
                    text: "View Receipt",
                    onPress: () =>
                      navigation.navigate("Receipt", { bookingId: bookingId }),
                  },
                  {
                    text: "Stay Here",
                    style: "cancel",
              ...
              Alert.alert(
                "Error",
                `Failed to complete booking: ${error.message || error}`
              );
            }
          },
        },
  ...
  const renderTimeDisplay = (bookingId) => {
    ...
    return (
      ...
      >
        ...
        {isCompleted && (
          <View style={[styles.frozenIndicator, { backgroundColor: theme.success }]}>
            <Ionicons name="pause-circle" size={16} color="#fff" />
            <Text style={[styles.frozenText, { color: "#fff" }]}>
              FROZEN
            </Text>
          </View>
        )}
